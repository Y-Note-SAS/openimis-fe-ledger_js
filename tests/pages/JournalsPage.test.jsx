...
const journalRef = vi.hoisted(() => ({ current: null }));
const searcherJournal = () => journalRef.current;
...
vi.mock("../../src/components/JournalsSearcher", () => ({
  default: ({ canManage, onCreate, onEdit, onDelete }) => (
    <div data-testid="journals-searcher">
      ...
      <button type="button" onClick={() => onEdit(searcherJournal())}>
        open-edit-dialog
      </button>
      <button type="button" onClick={() => onDelete(searcherJournal())}>
        ask-delete
      </button>
    </div>
  ),
...
describe("JournalsPage", () => {
  ...
  it("edits a journal with its uuid and closes the dialog on success", async () => {
    ...
    // The HTTP response is not a success signal: the dialog waits for the
    // mutation-log confirmation.
    ...
    store.dispatch({ type: `${ACTION_TYPE.UPDATE_JOURNAL}_CONFIRMED` });

    await waitFor(() => expect(screen.queryByLabelText("ledger.journals.form.name")).not.toBeInTheDocument());
  });

  it("keeps the dialog open and shows the rejection reported by the mutation log", async () => {
    const { actions } = renderPage(store);
    ...
    store.dispatch({ type: `${ACTION_TYPE.UPDATE_JOURNAL}_RESP`, payload: { data: { updateJournal: {} } } });
    store.dispatch({
      type: `${ACTION_TYPE.UPDATE_JOURNAL}_ERR`,
      payload: {
        status: 400,
        statusText: "The specified journal to delete was not found",
        message: "The specified journal to delete was not found",
      },
    });

    await waitFor(() => expect(screen.getByText(/The specified journal to delete was not found/)).toBeInTheDocument());
    expect(screen.getByLabelText("ledger.journals.form.name")).toBeInTheDocument();
    expect(actions.updateJournal).toHaveBeenCalledTimes(1);
  });
