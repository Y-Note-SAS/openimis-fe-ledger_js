...
describe("JournalsSearcher", () => {
  ...
  it("renders the translated journal type and the default accounts", () => {
    ...
    // Content-based: a column reorder must not break this test.
    const { container } = renderRow(props, journal);
    const row = container.textContent;
    expect(row).toContain("Banque");
    expect(row).toContain("5120 — Banque");
    expect(row).toContain("7010 — Ventes");
  });
    ...
    const { container: row } = render(
      <>
        {formatters.map((format, index) => (
          <span key={index}>{format(props.items[0] ?? {})}</span>
        ))}
      </>,
    );
    expect(row.querySelector('[aria-label="ledger.journals.actions.delete"]')).toBeNull();
    expect(row.querySelector('[aria-label="ledger.journals.actions.edit"]')).toBeNull();
