...
describe("JournalFormDialog", () => {
  ...
  it("keeps the save button disabled until every required field is filled", () => {
    ...
    fireEvent.change(selectFor("ledger.journals.form.defaultDebitAccount"), { target: { value: "acc-1" } });
    fireEvent.change(selectFor("ledger.journals.form.defaultCreditAccount"), { target: { value: "acc-2" } });
    expect(save).toBeDisabled();

    fireEvent.change(selectFor("ledger.picker.journalType"), { target: { value: "bank" } });
