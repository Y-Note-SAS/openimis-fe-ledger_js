...
/* Single form for both creation and edition of a ledger account (ticket 37991).
   ...
   `fullCode` is deliberately absent: it is derived by the backend (concatenation
   of the codes of the parent chain) and is no longer accepted as an input.

   The parent may only be chosen when creating: `updateAccount` writes the new
   parent with `QuerySet.update()`, which leaves the MPTT tree (lft/rght/tree_id/
   level) inconsistent, so re-parenting is not offered until the backend is
   fixed. On edition the current parent is displayed read-only. */
const AccountFormDialog = ({
  ...
  // Any account with a parent has its type imposed by the DB trigger — in
  // edition too, so the field stays locked there as well.
  const parentDrivenType = Boolean(form.parent);
  // hordak: `bank_accounts_are_asset_accounts`. Under a non-asset parent the
  // box must stay unavailable, otherwise re-ticking it would ask the backend
  // for a type the parent overrides (IntegrityError instead of a clear error).
  const bankAccountLocked = parentDrivenType && form.parent?.type !== ACCOUNT_TYPE.ASSET;
  ...
  // A child account inherits its parent's type: the DB trigger
  // `check_account_type` overwrites it on write, so the field is displayed
  // locked instead of letting the user send a value that would be ignored.
  const setParent = (parent) =>
    setForm((previous) => ({
      ...previous,
      parent,
      type: parent?.type || previous.type,
      isBankAccount: parent && parent.type !== ACCOUNT_TYPE.ASSET ? false : previous.isBankAccount,
    }));
  const canSave = Boolean(form.name && form.code && form.type && form.currencies.length);
      ...
      <DialogContent dividers>
        {error ? (
          <Alert severity="error">
            {formatMessage(intl, "ledger", "ledger.accounts.rejectionTitle")}: {mutationMessage(intl, error)}
          </Alert>
        ) : null}
        <Grid container spacing={2}>
          ...
          <Grid size={6} className="item">
            <AccountPicker
              label={formatMessage(intl, "ledger", "ledger.accounts.form.parent")}
              value={form.parent}
              onChange={setParent}
              readOnly={isEdit}
            />
          ...
          <Grid size={6} className="item">
            <AccountTypePicker
              value={form.type}
              readOnly={form.isBankAccount || parentDrivenType}
